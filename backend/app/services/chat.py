...
def _summarize_items_for_prompt(itinerary: ItineraryVersion | None) -> str:
    ...
    preview = sorted(itinerary.items, key=lambda item: (item.date, item.start_time))[:8]
    lines = []
    for item in preview:
        lines.append(f"- id={item.id} | {item.date} {item.start_time}-{item.end_time} | {item.title}")
    return "\n".join(lines)
...
def build_chat_response(trip: Trip, itinerary: ItineraryVersion | None, places: list[Place], user_message: str) -> ChatResponse:
    ...
    prompt = (
        ...
        '  "proposed_changes": [\n'
        "    {\n"
        '      "type": "update_item|generate_itinerary",\n'
        '      "title": "titulo curto",\n'
        '      "reason": "justificativa curta",\n'
        '      "payload": {"item_id": 123, "title": "novo titulo", "notes": "nova nota", "start_time": "09:30:00", "end_time": "11:00:00"}\n'
        "    }\n"
        "  ],\n"
        '  "warnings": ["..."]\n'
        "}\n\n"
