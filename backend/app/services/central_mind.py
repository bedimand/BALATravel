    ...
    def _get_mode_instructions(self, db: Session, context: MindContext, active: ItineraryVersion | None) -> str:
        if context.mode == "autonomous":
            ...
            return (
                ...
                "2. Call review_itinerary. It returns, per day, the timeline and a list of issues tagged 'blocking',\n"
                "   'warning', or 'critique' (a separate expert's judgment on real-world sense — e.g. a restaurant\n"
                "   or bar at 09:00, monotony, bad pacing), plus the trip-wide blocking_count.\n"
                "3. READ every issue in the report (the full list is given to you). Fix each blocking issue (search for\n"
                "   a missing anchor, add the missing lunch/dinner, swap a repeated venue, or rebalance with set_day).\n"
                "   Also act on 'critique' issues when sensible — e.g. swap a 09:00 restaurant for a café. Then call\n"
                "   review_itinerary again.\n"
